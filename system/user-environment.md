...
```bash
$ ssh -l <사용자ID> neuron.ksc.re.kr [-P 22]
```

* 예) 사용자ID가 x123abc일 경우

```bash
$ ssh -l x123abc neuron.ksc.re.kr
혹은
$ ssh -l x123abc neuron.ksc.re.kr -P 22
```
...
```bash
$ ssh -l x123abc neuron.ksc.re.kr
(x123abc@neuron.ksc.re.kr) Password(OTP): OTP 6자리 숫자 토큰
(x123abc@neuron.ksc.re.kr) Password: 슈퍼컴퓨터 로그인 비밀번호
```
...
*   사용자 비밀번호를 변경하는 방법은 누리온에 접속 한 후 passwd 명령어를 통한 변경과 사용자 포털(www.ksc.re.kr)을 통한 변경  2가지 방법이 제공됩니다.&#x20;
    ...
    * 사용자 패스워드를 변경하기 위해서는 뉴론에 슈퍼컴퓨터 아이디를 통해 로그인 한 후 passwd 명령을 사용합니다.
